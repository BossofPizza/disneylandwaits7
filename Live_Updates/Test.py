...
ride_names = ["Big Thunder Mountain Railroad", "Space Mountain"]
...
# Streamlit app to show the results
st.title("Disneyland Ride Wait Time Predictions")

# Button to run the update
if st.button("Check Ride Wait Times"):
    # Loop through each ride and print wait time with category
    for ride_name in ride_names:
        current_wait_time = get_current_wait_time(ride_name)

        if current_wait_time is None:
            st.write(f"Wait time not available for {ride_name}.")
        else:
            ...
            st.write(f"{ride_name} - Actual Wait Time: {current_wait_time} minutes")
            st.write(f"Predicted Wait Time: {predicted_time:.2f} minutes")
            st.write(f"Category: {category} time to go")
